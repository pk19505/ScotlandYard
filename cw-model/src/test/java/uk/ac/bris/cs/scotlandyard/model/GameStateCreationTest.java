...
public class GameStateCreationTest extends ParameterisedModelTestBase {
	...
	@Test public void testGetRoundsMatchesSupplied() {
		GameState state = gameStateFactory.build(
				new GameSetup(standardGraph(), ImmutableList.of(true, false, true, true)),
				blackPlayer(),
				bluePlayer());
		assertThat(state.getSetup().rounds).containsExactly(true, false, true, true);
	}
	...
	@Test public void testGetPlayersMatchesSupplied() {
		GameState state = gameStateFactory.build(standard24RoundSetup(),
				...
				greenPlayer(),
				bluePlayer());
		assertThat(state.getPlayers()).containsExactly(MRX, RED, GREEN, BLUE);
	}
	...
	@Test public void testGetPlayerTicketsMatchesSupplied() {
		...
		GameState game = gameStateFactory.build(standard24RoundSetup(), mrX, blue);
